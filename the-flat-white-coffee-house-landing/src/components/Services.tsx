...
export default function Services() {
    ...
    const menuItems = services.map((service, idx) => {
        ...
        if (service.toLowerCase().includes("coffee") || service.toLowerCase().includes("espresso") || service.toLowerCase().includes("flat white")) {
            icon = Coffee;
            category = "Beverages";
        } else if (service.toLowerCase().includes("sandwich") || service.toLowerCase().includes("bakery")) {
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {menuItems.map((item, idx) => {
                        ...
                        return (
                            ...
                            >
                                <div className="mb-6 p-4 bg-amber-50 w-fit rounded-2xl group-hover:bg-amber-800 group-hover:text-white transition-colors duration-500">
                                    <Icon size={40} />
                                </div>
                                <div className="mb-4">
                ...
                >
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                        {cafeData.positioning.best_for.map((tag, idx) => (
                            <span
                                key={idx}
                                className="px-6 py-3 bg-amber-50 text-amber-800 rounded-full font-bold text-sm border border-amber-200"
                            >
                                {tag}
                            </span>
                        ))}
