...
import heroImage from "@/assets/goddip cafe .jpg";
    ...
    return (
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
            <div className="absolute inset-0 z-0">
                <Image
                    src={heroImage}
                    alt="Gossip Cafe - A Popular Hangout Spot in Vesu"
                    fill
                    className="object-cover"
                    priority
                    quality={90}
                />
                <div className="absolute inset-0  from-purple-900/80 via-purple-800/70 to-pink-900/80"></div>
                    ...
                    <h1 className="text-5xl md:text-8xl font-black text-white leading-tight mb-8 tracking-tighter">
                        {headline.includes("–") ? (
                            <>
                                ...
                                <span className="text-pink-400">{headline.split("–")[1]?.trim()}</span>
                            </>
                        ) : (
                            headline
                        )}
