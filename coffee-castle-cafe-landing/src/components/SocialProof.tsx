      ...
      <div className="container mx-auto max-w-6xl">
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 sm:gap-8 md:gap-16">
          ...
          <div className="flex-1 max-w-2xl w-full">
            <h3 className="text-lg sm:text-xl font-semibold text-amber-900 mb-4 text-center md:text-left">
            ...
            <div className="space-y-3 sm:space-y-4">
              {cafeData.social_proof.testimonial_quotes.map((quote, index) => (
                ...
                >
                  <p className="text-sm sm:text-base text-stone-700 italic">&quot;{quote}&quot;</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-8 sm:mt-12 pt-6 sm:pt-8 border-t border-stone-200">
          <h4 className="text-base sm:text-lg font-semibold text-amber-900 mb-3 sm:mb-4 text-center">
            Top Review Themes
          </h4>
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
            {cafeData.social_proof.top_review_themes.map((theme, index) => (
              <span
                key={index}
                className="px-3 py-1.5 sm:px-4 sm:py-2 bg-amber-100 text-amber-800 rounded-full text-xs sm:text-sm font-medium"
              >
                {theme}
              </span>
            ))}
