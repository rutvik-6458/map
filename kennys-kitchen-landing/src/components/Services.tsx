...
export default function Services() {
  ...
  return (
    <section id="services" className="py-12 sm:py-16 md:py-20 bg-gradient-to-b from-white to-orange-50/30">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-10 sm:mb-12 md:mb-16">
          {signatureItems.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-white p-5 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-100 hover:shadow-xl transition-all transform hover:-translate-y-1"
              >
                <div className="w-12 h-12 sm:w-14 sm:h-14 bg-orange-600 rounded-lg sm:rounded-xl flex items-center justify-center mb-3 sm:mb-4 shadow-lg shadow-orange-200">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-12 shadow-xl border border-gray-100">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {features.map((feature, index) => (
              <div
                key={index}
                className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 bg-orange-50 rounded-lg sm:rounded-xl border border-orange-100"
              >
                <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-orange-600 flex-shrink-0"></div>
          ...
          <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-gray-200 flex flex-col sm:flex-row flex-wrap items-center justify-center gap-4 sm:gap-6">
            <div className="flex items-center gap-2 text-gray-700">
              <Clock size={18} className="sm:w-5 sm:h-5 text-orange-600 flex-shrink-0" />
              <span className="font-semibold text-sm sm:text-base">Open daily, closes at 12 am</span>
            </div>
            <div className="flex items-center gap-2 text-gray-700">
              <Truck size={18} className="sm:w-5 sm:h-5 text-orange-600 flex-shrink-0" />
              <span className="font-semibold text-sm sm:text-base">Dine-in • Takeaway • Delivery</span>
            </div>
          </div>
        </div>
      </div>
